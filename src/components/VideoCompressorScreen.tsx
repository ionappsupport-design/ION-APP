      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950/80 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Reclaim up to 70% space per video</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {videoFiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-300">No Videos Found</h3>
          ...
          <>
              ...
              <div className="flex gap-2.5 overflow-x-auto pb-2 scrollbar-none">
                {videoFiles.map((vid) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-8 h-8 rounded-xl bg-slate-900/80 flex items-center justify-center shrink-0">
                        <Video className="w-4 h-4 text-blue-400" />
                      </div>
                      <div className="max-w-[120px]">
                        <p className="text-xs font-bold truncate">{vid.name}</p>
                        <p className="text-[11px] text-slate-400">{formatBytes(vid.size)}</p>
                      </div>
                    </button>
                  );
                })}
            ...
            {selectedVideo && (
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-3xl p-5 space-y-4 shadow-xl">
                {/* Active Video Info */}
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Original Size: <strong className="text-white">{formatBytes(selectedVideo.size)}</strong>
                    </p>
                  </div>
                  <span className="px-2.5 py-1 rounded-full bg-blue-500/20 text-blue-300 text-[11px] font-semibold border border-blue-500/30">
                    MP4 / H.264
                  </span>
                </div>
                ...
                <div className="bg-emerald-950/40 border border-emerald-500/30 rounded-2xl p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-emerald-200">Estimated Space Saved:</span>
                  </div>
                ...
                {isCompressing && (
                  <div className="space-y-2 bg-slate-900/80 p-4 rounded-2xl border border-slate-700">
                    <div className="flex justify-between text-xs font-bold">
                      <span className="text-slate-300">Compressing video frames...</span>
                ...
                {completedResult && (
                  <div className="bg-emerald-950/60 border border-emerald-500/40 rounded-2xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
                      <div>New Size: <strong className="text-white">{formatBytes(completedResult.actualSize)}</strong></div>
                      <div>Space Recovered: <strong className="text-emerald-400">{formatBytes(completedResult.savingsBytes)}</strong></div>
                    </div>
                ...
                {!completedResult && (
                  <button
                    ...
                    className="w-full py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-xl shadow-blue-600/20 transition-all"
                  >
